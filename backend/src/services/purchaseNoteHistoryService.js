/**
 * Service for managing purchase note generation history
 */

/**
 * Check if purchase manager has already generated a note for a vehicle
...
const hasUserGeneratedNote = (vehicle, userId) => {
  ...
  return vehicle.purchaseNoteHistory.some(entry => {
...
/**
 * Add a new purchase note history entry
 * @param {Object} vehicle - Vehicle document
...
const addHistoryEntry = async (vehicle, userId, vehicleNo) => {
  const filename = `Purchase_Note_${vehicleNo}_${Date.now()}.pdf`
  
  if (!vehicle.purchaseNoteHistory) {
    vehicle.purchaseNoteHistory = []
  }
  
  vehicle.purchaseNoteHistory.push({
    generatedBy: userId,
...
const shouldSaveHistory = async (vehicle, user, isDownloadOnly) => {
  ...
  // Purchase managers can only generate once per vehicle
...
/**
 * Save purchase note history if conditions are met
 * @param {Object} vehicle - Vehicle document
